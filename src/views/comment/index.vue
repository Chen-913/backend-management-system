<template>
  <div class="app-container">
    <h1>评论列表</h1>
    <!-- 评论表格 -->
    <el-table v-loading="loading" border :data="commentList" style="width: 100%">
      ...
      <el-table-column label="评论的文章" width="200" align="center">
        <template slot-scope="scope">
          {{ scope.row.blog.title }}
        </template>
      </el-table-column>
      <el-table-column prop="content" label="评论内容" min-width="200" align="center" />
      <el-table-column prop="createDate" label="评论日期" min-width="220" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    fetchData() {
      // 获取数据
      this.loading = true;
      getComments(this.pagination.currentPage, this.pagination.pageSize).then(r => {
    ...
    formatDate,
    handleDelete(commentInfo) {
      this.$confirm('该操作不可逆，确定删除该评论吗?', '删除评论', {
        type: 'warning'
      }).then(r => {
        deleteComment(commentInfo.id).then(r => {
          this.$message({
            type: 'success',
            message: '删除成功'
          });
